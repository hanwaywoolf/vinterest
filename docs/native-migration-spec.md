...
4. **The pricing backend was unwired, and is now gone.** ~~Apify (Wine-Searcher scraper), LCBO, and the Supabase `wine_cache`~~ were built server-side but never called by the client — every price the user saw was already a live Claude estimate. Resolved: the dead routes (Apify, the Supabase `wine_cache` cache, and LCBO before it) have been deleted along with the rest of `functions/`. Retail/price is Claude-only going forward — the label-scan prompt's `price_usd` field and `fetchRetailEstimate` in `pwa-components.jsx`, labeled as estimates in the UI.
